...
router.get("/get_web_public", async (req, res) => {
  try {
    ...
    const finalAddon = returnAddons();
    const config = data[0] ? { ...data[0] } : {};

    // If OAuth IDs are placeholders/dummy, set active to 0 to prevent 401 error popups
    if (
      !config.google_client_id ||
      config.google_client_id.startsWith("xxx") ||
      config.google_client_id.length < 15
    ) {
      config.google_login_active = 0;
    }

    if (
      !config.fb_login_app_id ||
      config.fb_login_app_id.startsWith("xxx") ||
      isNaN(Number(config.fb_login_app_id))
    ) {
      config.fb_login_active = 0;
    }

    res.json({ data: config, addon: finalAddon || [], success: true });
  } catch (err) {
    res.json({ success: false, error: err, msg: "Server error", err });
    logger.log(err);
  }
