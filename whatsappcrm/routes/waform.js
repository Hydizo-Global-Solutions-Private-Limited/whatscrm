  ...
  async (req, res) => {
    try {
      ...
      const isLiveMeta =
        ...
        !metaApi.access_token.startsWith("xxx") &&
        metaApi.access_token.length > 20 &&
        metaApi.waba_id &&
        !metaApi.waba_id.startsWith("xxx");

      if (!isLiveMeta) {
        return res.json({
          success: true,
          msg: "Loaded local WhatsApp forms",
          synced: 0,
        });
      }
      ...
      logger.error("Meta sync-forms error:", err?.response?.data || err.message);
      res.json({
        success: true,
        msg: "Loaded local WhatsApp forms",
        synced: 0,
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        msg: isLiveMeta
          ? "Form created and published to Meta successfully"
