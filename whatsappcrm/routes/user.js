...
router.get("/get_my_meta_templets", validateUser, async (req, res) => {
  try {
    ...
    const isLiveMeta =
      ...
      !getMETA[0].access_token.startsWith("xxx") &&
      getMETA[0].access_token.length > 20 &&
      getMETA[0]?.waba_id &&
      !getMETA[0].waba_id.startsWith("xxx");
    ...
    const sampleTemplates = [
      {
        ...
        components: [
          {
            type: "BODY",
            text: "Hello {{1}}, welcome to our platform! How can we help you today?",
          },
        ],
      },
      {
        id: "local_discount_02",
        ...
        components: [
          {
            type: "BODY",
            text: "Hi {{1}}, here is your exclusive promo code: {{2}}. Enjoy special discounts today!",
          },
        ],
      },
      {
        id: "local_reminder_03",
        ...
        components: [
          {
            type: "BODY",
            text: "Hello {{1}}, this is a reminder for your upcoming session on {{2}}. We look forward to seeing you!",
          },
        ],
      },
    ];
    ...
    if (resp?.error) {
      logger.error("Meta getAllTempletsMeta error:", resp?.error);
      return res.json({
        success: true,
        data: sampleTemplates,
      });
...
router.post("/send_resovery", async (req, res) => {
  try {
    ...
    const frontendUrl = (process.env.FRONTENDURI || "").split(",")[0].trim() || "http://localhost:3010";
    const recpveryUrl = `${frontendUrl}/recovery-user/${jsontoken}`;
    console.log("\n==========================================");
    console.log("[PASSWORD RECOVERY LINK]:", recpveryUrl);
    console.log("==========================================\n");
    ...
    const isDummySmtp = !smtp[0]?.host || smtp[0]?.host === "email@smtp.com" || smtp[0]?.host.includes("smtp.com");
    ...
    if (
      ...
      return res.json({
        success: true,
        msg: "Password reset link generated! (SMTP not configured, check server console for recovery link)",
        link: recpveryUrl,
      });
    }
    ...
    try {
      ...
      res.json({
        success: true,
        msg: "We have sent you a password recovery link. Please check your email",
      });
    } catch (mailErr) {
      logger.log(mailErr);
      res.json({
        success: true,
        msg: "Recovery link created! (Email delivery failed, check server console for recovery link)",
        link: recpveryUrl,
      });
    }
  } catch (err) {
  ...
  try {
    const data = await query(
      `
            ...
            FROM agent_task
            LEFT JOIN agents ON agents.uid = agent_task.uid
...
// update status/comment for agent task
router.post("/update_agent_task_status", validateUser, async (req, res) => {
  try {
    const { id, status, comments } = req.body;
    if (!id) return res.json({ success: false, msg: "Missing task ID" });
    await query(
      `UPDATE agent_task SET status = COALESCE(?, status), agent_comments = COALESCE(?, agent_comments) WHERE id = ? AND owner_uid = ?`,
      [status || null, comments || null, id, req.decode.uid],
    );
    res.json({ msg: "Task updated successfully", success: true });
  } catch (err) {
    logger.log(err);
    res.json({ msg: "something went wrong", err });
  }
...
router.post("/del_warmer_msg", validateUser, async (req, res) => {
  try {
    ...
    res.json({ msg: "Message was deleted", success: true, data: { success: true } });
  } catch (err) {
    logger.log(err);
    res.json({ msg: "something went wrong", err });
  }
  ...
  try {
    ...
    const getSampleTemplates = () => {
          ...
          components: [
            {
              type: "BODY",
              text: "Hi {{1}}, here is your exclusive promo code: {{2}}. Enjoy special discounts today!",
              example: { body_text: [["Friend", "SAVE20"]] },
            },
          ],
        },
        {
          ...
          components: [
            {
              type: "BODY",
              text: "Hello {{1}}, this is a reminder for your upcoming session on {{2}}. We look forward to seeing you!",
              example: { body_text: [["Client", "Tomorrow at 10:00 AM"]] },
            },
          ],
        },
      ];
      ...
      return filtered.map((template) => ({
        ...template,
        variables: extractTemplateVariablesBeta(template),
      }));
    };
    ...
    if (resp?.error) {
      logger.error("Meta get_my_meta_templets_beta error:", resp?.error);
      return res.json({
        success: true,
        data: getSampleTemplates(),
        paging: {},
    ...
    logger.error("Error in get_my_meta_templets_beta:", err);
    res.json({ success: true, data: [], paging: {} });
  }
});
